...
#define __PC_ROM	0x200	// saved for rom instructions
#define __ZERO		0		// general purpose for clear values that are equal to 0

#pragma region Chip8.H integration
...
void chip8::initilize()
{
	...
	pc			= __PC_ROM;		// Program counter start
	opcode		= __ZERO;		// Reset current opcode
	I			= __ZERO;		// Reset index register
	sp			= __ZERO;		// Reset stack pointer

	for (int i = 0; i < sizeof(gfx);	i++)	{ gfx[i] = 0; } 	// Clear display
	for (int i = 0; i < sizeof(stack);	i++)	{ stack[i] = 0;	}	// Clear stack
	for (int i = 0; i < sizeof(key);	i++)	{ key[i] = 0; }		// Clear registers V0-VF
	for (int i = 0; i < sizeof(memory); i++)	{ memory[i] = 0; }	// Clear memory
...
bool chip8::loadApplication(const char * filename)
{
	...
	if ((sizeof(memory) - __PC_ROM) > lSize)
	{
		for (int i = 0; i < lSize; ++i)
		{
			memory[i + __PC_ROM] = buffer[i];
		}
	}
	else
	{
...
bool chip8::loadApplication(const char * filename)
{
	...
	//open file
	FILE* appFile = fopen(filename, "rb"); //binary mode
